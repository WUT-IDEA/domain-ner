function STH_BPMLL(dataset, alg)
...
%%
codeLen = 2:2:20;        % 编码长度（1*16的矩阵），从4开始到64为止的等差数列，公差是4
...
codeTest1(codeTest1 == 0) = -1;
% use BP_MLL to train a neural network
[nets,errors]=BPMLL_train(feaTrain, codeTrain1', 40);
[HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Threshold,Pre_Labels]=BPMLL_test(feaTrain,codeTrain1',feaTest,codeTest1',nets);
Pre_Labels(Pre_Labels == -1) = 0;
codeTest = Pre_Labels';
% matrix2txt(test_labels,'test_labels');
% matrix2txt(test_outputs,'test_outputs');
