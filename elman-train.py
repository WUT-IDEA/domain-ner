...
from is13.examples.prepare_data_for_rnn import label2idx,dictionary,get_datalist
...
if __name__ == '__main__':
    ...
    alltrain_x =get_datalist('../data/trainx.txt')
    alltrain_y  =get_datalist('../data/trainy.txt')
    # test_lex =get_data list('../data/zhengzhi_idx.txt')
    train_lex=alltrain_x[0:15587]
    test_lex=alltrain_x[15588:]
    train_y = alltrain_y[0:15587]
    test_y = alltrain_y[15588:]
    ...
    for e in xrange(s['nepochs']):
        ...
        if res_test['f1'] > best_f1:
            ...
            if s['verbose']:
                tempstr= 'NEW BEST: epoch '+str(e)+', best test F1, P ,R '+str(res_test['f1'])+str(res_test['p'])+str(res_test['r'])
                f = open('result.txt', 'a')
                f.write(tempstr + '\n')
                f.close()
    ...
    zhengzhi_idx = get_datalist('../data/zhengzhi_idx.txt')
    print 'length of zhengzhi txt sentences:', len(zhengzhi_idx)
    count = 0
    ...
    zhengzhi_predx = open('../data/zhengzhi_predx.txt', 'w') #zhengzhi_idx after sorted
    zhengzhi_predy = open('../data/zhengzhi_predy.txt', 'w')
    ...
    for sentence in sorted_dic:
        ...
        zhengzhi_predx.write(str(sentence[1][0])+'\n')
        zhengzhi_predy.write(str(sentence[1][1]) + '\n')
    zhengzhi_predx.close()
    zhengzhi_predy.close()
    print 'predict zhengzhi done!!!'
