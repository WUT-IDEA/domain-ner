...
from is13.rnn.my_elman33 import model
...
if __name__ == '__main__':
    ...
    # alltrain_x =get_datalist('is13/data/cotrain/tempRBFtrainx.txt')
    # alltrain_y  =get_datalist('is13/data/cotrain/tempRBFtrainy.txt')
    # train_lex=alltrain_x
    # train_y=alltrain_y
    # test_lex =get_datalist('is13/data/cotrain/testx.txt')
    # test_y = get_datalist('is13/data/cotrain/testy.txt')
    
    #initial running, obtain zhengzhi trainset
    alltrain_x = get_datalist('is13/data/human-trainx.txt')
    alltrain_y = get_datalist('is13/data/human-trainy.txt')
    ribao_x=get_datalist('sorted_xRBF_remove_repeat.txt')[0:2035]
    ribao_y = get_datalist('sorted_yRBF_remove_repeat.txt')[0:2035]
    train_lex=alltrain_x[0:2035]
    test_lex=alltrain_x[2035:]
    train_y = alltrain_y[0:2035]
    test_y = alltrain_y[2035:]
    ...
    for e in xrange(s['nepochs']):
        ...
        for i in xrange(nsentences):
            ...
            labels = train_y[i]
            ribao_labels=ribao_y[i]
            ribao_cwords = contextwin(train_lex[i], s['win'])
            ribao_words = map(lambda x: numpy.asarray(x).astype('int32'), minibatch(cwords, s['bs']))

            #print 'label=', labels
            for word_batch ,ribao, label_last_word,ribaoy in zip(words, ribao_words,labels,ribao_labels):
                t=rnn.train(word_batch, ribao,label_last_word,ribaoy, s['clr'])
                # rnn.normalize()
            if s['verbose']:
        ...
        predictions_test = [ map(lambda x: idx2label[x], \
                             rnn.classify(numpy.asarray(contextwin(x, s['win'])).astype('int32'),numpy.asarray(contextwin(x, s['win'])).astype('int32'))) for x in test_lex ]
    ...
    zhengzhi_idx = get_datalist('is13/data/zhengzhi_useful.txt')
    ...
    for x in zhengzhi_idx:
        tem=numpy.asarray(contextwin(x, s['win'])).astype('int32')
        scores = rnn.myclassify(tem,tem)  # each word's 127 lebels score in each line[[label1_score,label2_score,..][label1_score,label2_score,..]]
        maxscores = map((lambda x: listmax(x)), scores)
        sentence_score = sum(maxscores)/len(maxscores)
        sentence_label = map(lambda x: idx2label[x], rnn.classify(tem,tem))
        x = x.tolist()
        count += 1
    ...
    zhengzhi_predx = open('is13/data/exp3/pred_exp3x.txt', 'w') #zhengzhi_idx after sorted
    zhengzhi_predy = open('is13/data/exp3/pred_exp3y.txt', 'w')
